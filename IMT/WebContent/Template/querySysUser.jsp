			...
			<tbody>
				...
				<c:forEach items="${sysuserQuery}" var="item" varStatus="status">
					<tr>
						...
						<c:if test="${sessionScope.currstate==0||sessionScope.currstate==2}">
							无权查看
						</c:if>
						</td>
						<c:if test="${currstate==3}">
						...
						<div>
							...
							<c:if test="${item.sys_user_able==1&&item.sys_user_state==0}">
							<input type="button"  value="禁用" name="${item.sys_user_id}" onclick="editb(this)">
							</c:if>
							<c:if test="${item.sys_user_able==0&&item.sys_user_state==0}">
