...
<body>
	...
	<table>
		<tr>
			...
			<td><input type="text" name="Cname" value="${updateuser.sys_user_Cname}"></td>
		</tr>
		<tr>
			<td>用户名</td>
			<td>${updateuser.sys_user_name}</td>
			<td><input type="text" name="name" value="${updateuser.sys_user_name}"></td>
		</tr>
		<tr>
			<td>密   码</td>
			<td>${updateuser.sys_user_password}</td>
			<td><input type="text" name="password" value="${updateuser.sys_user_password}"></td>
		</tr>
		<tr>
			<td>
	...
	$('input[name="button1"]').bind("click",function(){
		...
		var id=${updateuser.sys_user_id};
		var mess={
				user_id:id,
				sys_user_Cname:tcname,
				sys_user_name:tname,
				sys_user_password:tpword
		}
		alert(111);
		$.ajax({
			type: "POST",
